...
document.addEventListener("DOMContentLoaded", function() {
    // Rotating titles script
    // const titles = ["Designer", "Art Director", "Cinematographer"];
// let index = 0;
// setInterval(() => {
...
// Set static title once
document.getElementById("title").textContent = "Creative Director & Filmmaker";
    ...
    function showTitle() {
        ...
        requestAnimationFrame(() => {
            setTimeout(() => {
                rotatingElement.style.transition = 'transform 0.5s ease, opacity 0.5s ease';
                rotatingElement.style.transform = 'translateY(0)';
                rotatingElement.style.opacity = 1;
            }, 100);
        });

        setTimeout(hideTitle, currentIndex === 0 ? initialPause : pauseBetweenTitles);
    }
   ...
   // ============================================================================
	...
	if ($("#page-header").hasClass("ph-content-parallax")) {
		...
		if ($(".ph-categories").length) {
			...
			tlPhParallax.to(".ph-cat-parallax", { y: -50 }, 0); // Changed from -80 to -50
		}
		if ($(".ph-caption-title").length) {
			$(".ph-caption-title").wrapInner('<div class="ph-title-parallax"></div>');
			tlPhParallax.to(".ph-title-parallax", { y: -45 }, 0); // Changed from -40 to -45
		}
...
$(".anim-fadeinup").each(function() {
    ...
    tl_FadeInUp.from(this, { 
        ...
        y: 100, 
        ease: Expo.easeOut
        // Removed clearProps:"all" - this was causing the jump
    }, "+=0.3");
});
...
$(".pgi-image-wrap").on("mouseenter", function() {
    $(this).find("video").each(function() {
        ...
        $(this).removeClass('fade-out').addClass('fade-in'); // Add fade-in class
        setTimeout(function() {
            video.play();
        }, 500); // Wait for the fade-in transition
    }); 
}).on("mouseleave", function() {
    $(this).find("video").each(function() {
        var video = $(this).get(0);
        $(this).removeClass('fade-in').addClass('fade-out'); // Add fade-out class

        setTimeout(function() {
            video.pause();
            video.currentTime = 0; // Reset to the start to show the poster image
            video.load(); // Reload the video to ensure the poster image is displayed
            $(video).removeClass('fade-out'); // Ensure fade-out class is removed after reset
   ...
   // ========================================
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		// Simulating loading progress (replace this with actual loading logic)
		let progress = 0;
